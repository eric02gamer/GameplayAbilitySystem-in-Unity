...
namespace EGF.Editor
{
    public class GameplayTagEditorWindow : EditorWindow
    {
        ...
        private void CreateGUI()
        {
            ...
            // 创建ScrollView和tag Container
            _scrollView = new ScrollView(ScrollViewMode.Vertical);
            _tagContainer = new VisualElement();
    
            // 设置ScrollView样式（可选）
            _scrollView.style.flexGrow = 1;  // 让ScrollView占满剩余空间// 设置ScrollView的其他样式属性
            _scrollView.style.marginTop = 5;
            _scrollView.style.marginBottom = 5;
            _scrollView.style.marginLeft = 5;
            _scrollView.style.marginRight = 5;
            // 如果需要设置最大高度
            // _scrollView.style.maxHeight = 300;
    
            // 将tagContainer添加到ScrollView中，再将ScrollView添加到root
            _scrollView.Add(_tagContainer);
