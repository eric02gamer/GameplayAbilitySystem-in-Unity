...
namespace EGF.Editor
{
    public class GameplayTagContainerView : VisualElement
    {
        ...
        // 界面引用
        private readonly ScrollView _tagContainer;
        ...
        // data
        private readonly SerializedProperty _targetPropertyRef;
        private readonly GameplayTagContainer _targetRef;
        private readonly SerializedGameplayTagData _serializedGameplayTagData;

        // 添加析构/清理方法
        ~GameplayTagContainerView()
        {
            UnregisterEvents();
        }
        ...
        public GameplayTagContainerView(string displayName, GameplayTagContainer target, SerializedProperty property)
        {
            // _instanceId = _nextInstanceId++;
            // Debug.Log($"[GameplayTagContainerView {_instanceId}] Creating new instance for {displayName}");
            
            var editorSetting = GameplayTagEditorSetting.GetOrCreateSettings();
            if (!editorSetting.CheckSettingReady())
            {
                Add(new HelpBox("Invalid editor setting", HelpBoxMessageType.Warning));
                return;
            }
            if (target == null)
            {
                Add(new HelpBox("Invalid property path", HelpBoxMessageType.Warning));
                return;
            }
            _targetRef = target;
            ...
            // create ui
            editorSetting.tagContainer.CloneTree(this);
            this.Q<Label>().text = displayName;

            _tagContainer = this.Q<ScrollView>();
            ResetTagContainer();
            ...
            _tagButton = this.Q<Button>("btn-show-detail");
        ...
        {
            // Debug.Log($"[GameplayTagContainerView {_instanceId}] OnAttachToPanel, target: {_targetRef?.GetHashCode()}");
            if (_targetRef != null)
            {
        ...
        {
            // Debug.Log($"[GameplayTagContainerView {_instanceId}] OnDetachFromPanel, target: {_targetRef?.GetHashCode()}");
            if (_targetRef != null)
            {
                _targetRef.ONNodeAdd -= HandleAddTag;
                _targetRef.ONNodeRemove -= HandleRemoveTag;
        ...
        {
            // Debug.Log($"[GameplayTagContainerView {_instanceId}] InitializeTagElements called, target: {_targetRef?.GetHashCode()}");
            // Debug.Log($"[GameplayTagContainerView {_instanceId}] InitializeTagElements called, TagContainerInitialized: {TagContainerInitialized}");
        ...
        {
            // Debug.Log($"[GameplayTagContainerView {_instanceId}] HandleAddTag called, TagContainerInitialized: {TagContainerInitialized}");
        ...
        {
            // Debug.Log($"[GameplayTagContainerView {_instanceId}] HandleRemoveTag called, TagContainerInitialized: {TagContainerInitialized}");
